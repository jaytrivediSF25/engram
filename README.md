> Anchored sliding window context for Claude Code.
...
**acw gives Claude Code long-term project memory that never forgets why the project exists.**
...
- **Opt-in per session** — `acw claude` gets the window; plain `claude` is completely untouched.
...
That's it. Work, quit, come back tomorrow, run `acw claude` again — the new session already knows where you left off *and* why the project exists.
...
A plain sliding window eventually evicts the sessions where you made the decisions that define the project. Full compaction keeps them, but grinds your recent work into lossy summary. acw spends exactly one tile of budget on a compacted anchor and keeps everything else at full fidelity.
...
1. `acw claude` launches Claude Code with `ACW_ACTIVE=1`. The installed SessionStart/SessionEnd hooks are **inert without it** — plain `claude` never touches acw.
...
|---|---|
| `acw claude [args…]` | Launch Claude Code with the anchored window (extra args pass through) |
| `acw status` | Show tiles, token counts, budget usage |
| `acw context` | Print the anchor + recent chats (`--json` for machines) |
| `acw add` | Manually store a chat (stdin, `--text`, `--file`) |
| `acw add --anchor` | Force content directly into the pinned anchor slot |
| `acw init` | Set per-project budgets up front |
| `acw reset` | Delete this project's store |

## Configuration

Budgets are per-project, set at `acw init` (or defaulted on first use):
...
```
~/.acw/projects/<hash-of-project-path>/
...
```sh
uv tool uninstall acw          # or: pipx uninstall acw
```

Then remove the two `acw hook-*` entries from `~/.claude/settings.json`
(a pre-install backup is kept at `settings.json.acw-backup`) and, if you
want the stored context gone, `rm -rf ~/.acw`.
